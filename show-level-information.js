...
(function () {
  ...
  const numberOfPowersFN = (book, books) => {
    for (let i = 0; i < books.length; i++) if (book === books[i]) return i;
  };
  ...
  const range = (level) => {
    if (team == "Orden del Fénix") {
      ...
      else if (between(level, 22, 36)) return "Templario";
      else if (between(level, 37, 55)) return "Knight";
      else if (between(level, 56, 60)) return "Demon Hunter";
    } else if (team == "Marca Tenebrosa") {
      if (between(level, 1, 9)) return "Base";
      else if (between(level, 10, 21)) return "Tempestad";
      else if (between(level, 22, 36)) return "Mago Oscuro";
      else if (between(level, 37, 55)) return "Nigromante";
      else if (between(level, 56, 60)) return "Ángel Caído";
    }
  };
  ...
  const maxNumberOfSkills = () => {
    ...
    if (currentLevel >= 20 && numberOfKnowledges >= 6) return "2";
    if (currentLevel >= 15 && numberOfKnowledges >= 5) return "1";
    return "No puedes adquirir habilidades";
  };

  let posts = parseInt(
    document
      .getElementsByClassName("row_data")[1]
      .childNodes[0].data.replace(".", "")
  );

  let labels = document.querySelectorAll(
    "#custom_fields_personaje ul li .row_title"
  );
  let data = document.querySelectorAll(
    "#custom_fields_personaje ul li .row_data"
  );
  ...
  levelList.appendChild(
    listDOM("Rango en el Bando (actual)", currentTeamRange)
  );
  levelList.appendChild(
    listDOM("Rango en el Bando (calculado)", calculatedTeamRange)
  );
  ...
  levelList.appendChild(
    listDOM("Experiencia para el próximo nivel", Math.round(nextLevel))
  );
  ...
  levelList.appendChild(
    listDOM("Máximo nivel de Criaturas", maxCreaturesLevel(currentLevel))
  );
